...
export function SocialLinks({ className = "", iconClassName = "h-5 w-5", showLabels = false }: SocialLinksProps) {
  ...
  return (
    <div className={`flex flex-wrap items-center gap-3 ${className}`}>
      {links.map(({ href, label, icon: Icon, className: itemClassName }) => (
        <a
          key={label}
          href={href}
          target="_blank"
          ...
          aria-label={label}
          className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold transition ${itemClassName}`}
        >
...
export function WhatsAppFab() {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      aria-label="راسلنا على WhatsApp"
      className="fixed bottom-24 left-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_12px_30px_rgba(37,211,102,0.45)] transition hover:scale-105 hover:bg-[#1ebe57] md:bottom-5"
    >
