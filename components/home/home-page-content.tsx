        ...
        <div className="container">
          <AnimatedBlock>
            <div className="max-w-3xl">
              <h2 className="text-4xl font-black">{home.painHeading}</h2>
              <p className="mt-4 text-lg leading-8 text-brand-muted">{home.painBody}</p>
            </div>
          </AnimatedBlock>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {home.painChips.map((item, index) => (
              <AnimatedBlock key={item.title} delay={index * 0.08}>
                <div className="rounded-[2rem] bg-brand-ivory p-6 transition-transform duration-300 hover:-translate-y-1 hover:shadow-sm">
                  ...
                  <p className="mt-2 text-brand-muted">{item.text}</p>
                </div>
              </AnimatedBlock>
            ))}
          </div>
